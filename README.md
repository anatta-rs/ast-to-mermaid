...
How does change to `pipeline::analyze` ripple outward? Every CLI binary funnels through `cli_support::run_analyze` — one diagram, the whole blast radius:

```mermaid
graph BT
    analyze(("fn analyze (impacted)"))
    a2m_function["a2m-function::main"]
    a2m_impact["a2m-impact::main"]
    a2m_module["a2m-module::main"]
    a2m_overview["a2m-overview::main"]
    a2m_project["a2m-project::main"]
    run_analyze["cli_support::run_analyze"]
    a2m_function --> run_analyze
    a2m_impact --> run_analyze
    a2m_module --> run_analyze
    a2m_overview --> run_analyze
    a2m_project --> run_analyze
    run_analyze --> analyze
```
...
Five zoom levels, one tool — `a2m-overview` and `a2m-function` are not shown above but follow the same shape.
